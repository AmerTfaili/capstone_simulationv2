...
from runnerDRQNV2 import DrqnV2Trainer


ROOT = Path(__file__).resolve().parent
DEFAULT_SCENARIO_DIR = ROOT / "sumo_benchmark"
DEFAULT_OUTPUT_DIR = ROOT / "runnerDRQNV220_outputs"
DEFAULT_CHECKPOINT_DIR = ROOT / "runnerDRQNV220_checkpoints"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Train the DRQN v2 baseline on the 20-intersection SUMO benchmark with isolated outputs."
    )
    ...
    parser.add_argument("--eval-label", type=str, default="DRQN v2 20", help="Label stored in the eval-only output JSON.")
    parser.add_argument("--eval-selection-method", type=str, default="CRRank + DRQN v2 20", help="Selection-method label stored in the eval-only output JSON.")
...
def main() -> None:
    ...
    print(f"runnerDRQNV220 scenario: {scenario_dir}")
    print(f"runnerDRQNV220 outputs: {output_dir}")
    print(f"runnerDRQNV220 checkpoints: {checkpoint_dir}")
    ...
    if args.eval_only:
        ...
        if args.eval_top_k is not None:
            ...
            print(f"DRQN v2 20 eval-only: using top-{args.eval_top_k} nodes = {trainer.controlled_tls}")
        trainer.run_single_evaluation(
            output_json_path=eval_output_path,
            checkpoint_path=resume_path,
            methodology="DRQN v2 20",
