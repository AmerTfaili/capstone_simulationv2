...
DEFAULT_OUTPUT = ROOT / "average_travel_time_attention_vs_no_attention_maps.png"
...
def load_episode_series(metrics_path: Path) -> tuple[list[int], list[float]]:
    ...
    values = [float(item["average_travel_time"]) for item in episode_metrics]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Plot average travel time for attention vs no-attention runnerMain on 50-node and 20-node maps."
    )
    ...
    fig.suptitle("Average Travel Time: Attention vs No-Attention Reward")
