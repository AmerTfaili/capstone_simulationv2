...
def load_checkpoint_config(checkpoint_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(config, dict):
        raise RuntimeError(f"Checkpoint does not contain a valid config block: {checkpoint_path}")
    return config
...
class DRQN(nn.Module):
    ...
    def forward(
        ...
        encoded = self.encoder(x.reshape(batch_size * seq_len, feat_dim))
        encoded = encoded.reshape(batch_size, seq_len, -1)
        recurrent_out, hidden = self.lstm(encoded, hidden)
        q_values = self.q_head(recurrent_out)
...
class SequenceReplayMemory:
    def __init__(self, capacity: int, sequence_length: int) -> None:
        ...
        self.sequences: deque[list[Transition]] = deque(maxlen=capacity)

    def add_episode(self, transitions: list[Transition]) -> None:
        if len(transitions) < self.sequence_length:
            return
        last_start = len(transitions) - self.sequence_length
        for start_index in range(last_start + 1):
            self.sequences.append(list(transitions[start_index : start_index + self.sequence_length]))

    def can_sample(self, batch_size: int) -> bool:
        return len(self.sequences) >= batch_size

    def sample(self, batch_size: int, device: torch.device) -> dict[str, torch.Tensor]:
        sampled_windows = random.sample(list(self.sequences), batch_size)
        states = np.stack([[transition.state for transition in window] for window in sampled_windows], axis=0)
        next_states = np.stack([[transition.next_state for transition in window] for window in sampled_windows], axis=0)
        actions = np.stack([[transition.action for transition in window] for window in sampled_windows], axis=0)
        rewards = np.stack([[transition.reward for transition in window] for window in sampled_windows], axis=0)
        dones = np.stack([[float(transition.done) for transition in window] for window in sampled_windows], axis=0)
        return {
    ...
    def to_serializable(self) -> list[list[dict[str, Any]]]:
        return [
            [
                {
            ...
            for window in self.sequences
        ]

    def load_serializable(self, payload: list[list[dict[str, Any]]]) -> None:
        ...
        for window in payload:
            ...
            self.sequences.append(restored_window)
...
class LivePlot:
    def __init__(self, output_path: Path) -> None:
        ...
        self.ax.set_title("Average Travel Time vs Episode")
    ...
    def update(self, train_metrics: list[dict[str, Any]], eval_metrics: list[dict[str, Any]]) -> None:
        self.train_line.set_data(
            [item["episode"] for item in train_metrics],
            [item["average_travel_time"] for item in train_metrics],
        )
        self.eval_line.set_data(
            [item["episode"] for item in eval_metrics],
            [item["average_travel_time"] for item in eval_metrics],
        )
        self.ax.relim()
...
class AgentRuntime:
    def __init__(
        ...
        self.online_net = DRQN(state_dim, config.hidden_size).to(self.device)
        self.target_net = DRQN(state_dim, config.hidden_size).to(self.device)
        self.target_net.load_state_dict(self.online_net.state_dict())
        self.optimizer = optim.Adam(self.online_net.parameters(), lr=config.learning_rate)
        self.replay = SequenceReplayMemory(config.replay_capacity, config.sequence_length)
    ...
    def select_action(self, state: np.ndarray, epsilon: float) -> int:
        ...
        with torch.no_grad():
            q_values, hidden = self.online_net(state_tensor, self.rollout_hidden)
        self.rollout_hidden = tuple(item.detach() for item in hidden)
        if random.random() < epsilon:
            return random.randint(0, 1)
        return int(torch.argmax(q_values[:, -1, :], dim=1).item())


class SumoDrqnTrainer:
    def __init__(self, config: TrainingConfig, resume_path: Path | None = None) -> None:
        self.config = config
        self.device = torch.device(config.device)
        roles = json.loads(CONTROL_ROLES_PATH.read_text(encoding="utf-8"))
        self.controlled_tls = list(roles["drqn_nodes"])
        self.net_path = self._resolve_sumocfg_path("net-file")
        self.route_path = self._resolve_sumocfg_path("route-files")
        self.fixed_tls = list(roles.get("fixed_time_nodes", []))
        if not self.fixed_tls:
            self.fixed_tls = [tls_id for tls_id in self._all_tls_ids_from_net() if tls_id not in self.controlled_tls]
    ...
    def override_controlled_tls(self, controlled_tls: list[str]) -> None:
        if not controlled_tls:
            raise RuntimeError("Controlled TLS override cannot be empty.")
        invalid = [tls_id for tls_id in controlled_tls if tls_id not in self.agents]
        if invalid:
            raise RuntimeError(f"Controlled TLS override contains unknown nodes for this checkpoint: {invalid}")
        self.controlled_tls = list(controlled_tls)
        self.agent_specs = {tls_id: self.agent_specs[tls_id] for tls_id in self.controlled_tls}
        self.agents = {tls_id: self.agents[tls_id] for tls_id in self.controlled_tls}
        self.fixed_tls = [tls_id for tls_id in self._all_tls_ids_from_net() if tls_id not in self.controlled_tls]
    ...
    def _all_tls_ids_from_net(self) -> list[str]:
        import xml.etree.ElementTree as ET

        root = ET.parse(self.net_path).getroot()
        return list(dict.fromkeys(node.attrib["id"] for node in root.findall("tlLogic") if "id" in node.attrib))
    ...
    def _select_major_green_states(self, phase_states: list[str]) -> list[str]:
        ...
        if not green_states:
            raise RuntimeError("Could not find any usable green states for a controlled traffic light.")
        if len(green_states) == 1:
            return [green_states[0], green_states[0]]
    ...
    def _make_simplified_logic(self, tls_id: str, green_states: list[str]) -> Any:
        ...
        return libsumo.trafficlight.Logic(f"drqn_{tls_id}", 0, 0, phases)
    ...
    def _capture_snapshot(self, agent: AgentRuntime) -> DecisionSnapshot:
        ...
        for phase_group in agent.phase_groups:
            incoming_queue = 0.0
            incoming_volume = 0.0
            incoming_speed_ratios: list[float] = []
            outgoing_queue = 0.0
            outgoing_occupancies: list[float] = []
            for lane_id in phase_group.incoming_lanes:
                incoming_queue += float(libsumo.lane.getLastStepHaltingNumber(lane_id))
                incoming_volume += float(libsumo.lane.getLastStepVehicleNumber(lane_id))
                mean_speed = float(libsumo.lane.getLastStepMeanSpeed(lane_id))
                max_speed = max(1.0, float(libsumo.lane.getMaxSpeed(lane_id)))
                incoming_speed_ratios.append(mean_speed / max_speed)
            for lane_id in phase_group.outgoing_lanes:
                outgoing_queue += float(libsumo.lane.getLastStepHaltingNumber(lane_id))
                occupancy = float(libsumo.lane.getLastStepOccupancy(lane_id)) / 100.0
                outgoing_occupancies.append(occupancy)
                if occupancy >= 0.78:
                    spillback_lanes += 1.0
            mean_speed_ratio = float(np.mean(incoming_speed_ratios)) if incoming_speed_ratios else 1.0
            mean_outgoing_occupancy = float(np.mean(outgoing_occupancies)) if outgoing_occupancies else 0.0
            pressure = incoming_queue - 0.55 * outgoing_queue
            total_queue += incoming_queue
            speed_terms.append(mean_speed_ratio)
            phase_pressures.append(pressure)
            phase_features.extend(
                [
                    min(2.0, incoming_queue / max(1.0, len(phase_group.incoming_lanes) * 5.0)),
                    min(2.0, incoming_volume / max(1.0, len(phase_group.incoming_lanes) * 7.0)),
                    mean_speed_ratio,
                    mean_outgoing_occupancy,
        ...
        state = np.array(
            phase_features + phase_one_hot + [phase_age, total_queue_norm, spillback_ratio],
            dtype=np.float32,
        )
    ...
    def _rollout(self, episode_index: int, epsilon: float, collect_experience: bool, sumo_seed: int) -> dict[str, Any]:
        ...
        try:
                ...
                if step % self.config.decision_interval == 0:
                    for agent in self.agents.values():
                        ...
                        if collect_experience and agent.pending_snapshot is not None and agent.pending_action is not None:
                            ...
                            agent.episode_transitions.append(
                                Transition(
                        ...
                        action = agent.select_action(snapshot.state, epsilon)
                        agent.pending_snapshot = snapshot
                        agent.pending_action = action
                        self._apply_action(agent.tls_id, action)
            ...
            if collect_experience:
                for agent in self.agents.values():
                    ...
                    agent.replay.add_episode(agent.episode_transitions)
    ...
    def _train_agent(self, agent: AgentRuntime) -> float | None:
        ...
        q_values, _ = agent.online_net(batch["states"])
        with torch.no_grad():
            next_q_values, _ = agent.target_net(batch["next_states"])
        loss = torch.zeros((), device=self.device)
        for step_index in range(self.config.sequence_length - 1, -1, -1):
            chosen_q = q_values[:, step_index, :].gather(1, batch["actions"][:, step_index].unsqueeze(1)).squeeze(1)
            target_q = next_q_values[:, step_index, :].max(dim=1).values
            td_target = batch["rewards"][:, step_index] + self.config.gamma * (
                1.0 - batch["dones"][:, step_index]
            ) * target_q
            loss = loss + torch.mean((td_target - chosen_q) ** 2)
        loss = loss / float(self.config.sequence_length)
    ...
    def _soft_update_target_network(self, agent: AgentRuntime) -> None:
        ...
        payload = {
            ...
            "agents": {
                tls_id: {
                    ...
                    "replay_sequences": agent.replay.to_serializable(),
                }
    ...
    def _load_checkpoint(self, checkpoint_path: Path) -> int:
        ...
        for tls_id, agent_payload in payload.get("agents", {}).items():
            ...
            replay_sequences = agent_payload.get("replay_sequences")
            if replay_sequences is not None:
                agent.replay.load_serializable(replay_sequences)
    ...
    def run_single_evaluation(
        ...
        methodology: str = "DRQN",
        selection_method: str | None = None,
        sumo_seed: int | None = None,
        label: str | None = None,
    ) -> dict[str, Any]:
        eval_episode = self.start_episode if self.start_episode > 0 else 1
        eval_seed = int(sumo_seed if sumo_seed is not None else self.config.sumo_seed + 1000)
        ...
        record = {
            ...
            "budget": len(self.controlled_tls),
            "scenario_dir": str(SCENARIO_DIR),
            "checkpoint_path": str(checkpoint_path) if checkpoint_path is not None else None,
            "checkpoint_episode": int(self.start_episode) if self.start_episode > 0 else None,
            "controlled_node_count": len(self.controlled_tls),
            "selected_nodes": list(self.controlled_tls),
        ...
        print(
            f"DRQN eval-only | nodes={len(self.controlled_tls)} | "
    ...
    def _print_train_progress(self, metrics: dict[str, Any], mean_loss: float | None) -> None:
        ...
        print(
            f"DRQN Train {metrics['episode']:>3}/{self.config.episodes} | "
    ...
    def _print_eval_progress(self, metrics: dict[str, Any]) -> None:
        avg_text = f"{metrics['average_travel_time']:.2f}" if not math.isnan(metrics["average_travel_time"]) else "nan"
        print(
            f"DRQN Eval  {metrics['episode']:>3}/{self.config.episodes} | "
            f"avg_tt={avg_text}s | inserted={metrics['inserted_ratio']:.3f} | "
    ...
    def train(self) -> dict[str, list[dict[str, Any]]]:
        ...
        for episode_index in range(self.start_episode, self.config.episodes):
            ...
            train_metrics = self._rollout(
                ...
                sumo_seed=self.config.sumo_seed,
            )
            losses: list[float] = []
            for agent in self.agents.values():
            ...
            if (episode_index + 1) % self.config.eval_interval == 0:
                eval_rollouts = [
                    self._rollout(
                        ...
                        epsilon=0.0,
                        collect_experience=False,
                        sumo_seed=self.config.sumo_seed + 1000 + rollout_index,
                    )
                ...
                eval_metrics = {
                    ...
                    "eval_seeds": [self.config.sumo_seed + 1000 + rollout_index for rollout_index in range(self.config.eval_episodes)],
                }
            ...
            if (episode_index + 1) % self.config.checkpoint_interval == 0:
                ...
                print(f"Saved DRQN checkpoint: {checkpoint_path}")

        final_checkpoint_path = self._save_checkpoint(self.config.episodes, is_final=True)
        print(f"Saved DRQN final checkpoint: {final_checkpoint_path}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Train independent DRQNs for SUMO traffic signal control.")
    ...
    parser.add_argument("--resume-from", type=Path, default=None, help="Resume training from a DRQN checkpoint path.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="SUMO seed used by --eval-only. Defaults to sumo_seed + 1000.",
    )
    parser.add_argument(
        "--eval-label",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="CRRank",
        help="Selection-method label stored in the eval-only output JSON.",
    )
    parser.add_argument(
        "--eval-top-k",
        type=int,
        default=None,
        help="For --eval-only, keep only the top-k nodes from the current control-roles / checkpoint order.",
    )
    return parser
...
def main() -> None:
    ...
    resume_path = resolve_path_from_root(str(args.resume_from)) if args.resume_from is not None else None
    ...
    if resume_path is not None:
        checkpoint_config = load_checkpoint_config(resume_path)
    ...
    trainer = SumoDrqnTrainer(config, resume_path=resume_path)
    if args.eval_only:
        ...
        if args.eval_top_k is not None:
            ...
            print(f"DRQN eval-only: using top-{args.eval_top_k} nodes = {trainer.controlled_tls}")
