...
def main() -> None:
    ...
    metadata_ids = metadata["signalized_intersection_ids"]
    report = {
        "total_non_internal_nodes_in_net": len(all_nodes),
        "signalized_intersections": len(tls_nodes),
        "signalized_ids": tls_nodes,
        "metadata_present": METADATA_PATH.exists(),
        "metadata_signalized_count": metadata["signalized_intersection_count"],
        "metadata_control_budget_target": metadata["control_budget_target"],
        "preselected_control_node_count": len(metadata["preselected_control_nodes"]),
        "metadata_matches_net_tls": sorted(metadata_ids) == sorted(tls_nodes),
