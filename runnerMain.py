...
OUTPUT_DIR = ROOT / "runnerMain_outputs_attention_reward"
CHECKPOINT_DIR = ROOT / "runnerMain_checkpoints_attention_reward"
...
class MethodologyRunner:
    ...
    def _attention_weighted_neighbor_value(
        self,
        node_index: int,
        values: np.ndarray,
        attention: np.ndarray,
    ) -> float:
        weights = np.array(attention[node_index], dtype=np.float32, copy=True)
    ...
    def _pressure_improvement_reward(
        ...
        if attention is not None:
            previous_neighbor = self._attention_weighted_neighbor_value(node_index, previous_pressures, attention)
            current_neighbor = self._attention_weighted_neighbor_value(node_index, current_pressures, attention)
    ...
    def _rollout_episode(
        ...
        try:
            ...
            while step < self.config.max_steps:
                curr_attention = None
                if self.frozen_gat_stats is not None and len(history_window) == self.config.history_length:
                    graph_outputs = self._graph_inference(list(history_window))
                    curr_attention = graph_outputs["attention"]
